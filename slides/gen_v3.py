...
# 简化：直接用单引号作为JS字符串分隔符的JS文件
# 这样双引号在里面出现就安全了

parts = []

parts.append('''const { Document, Packer, Paragraph, TextRun, Table, TableRow, TableCell,
...
# 组装完整文件
body_children = ''.join(content)

# 生成 bodySection 和 完整文档
body_section = f'''
const bodySection = {{
...
# 生成TOC条目
for item, color, level in toc_items:
    indent = 'left: 360' if level == 0 else 'left: 720'
    body_section += f'''    new Paragraph({{ spacing: {{ before: 80, after: 80 }}, indent: {{ {indent} }},
      children: [new TextRun({{ text: '{item}', size: 20, font: 'Microsoft YaHei', color: '{color}' }})] }}),
'''
...
# 写入文件
out_path = '/Users/wangcaihua/WorkBuddy/20260427142154/slides/build_product_manual_v3.js'
os.makedirs(os.path.dirname(out_path), exist_ok=True)
with open(out_path, 'w', encoding='utf-8') as f:
    f.write(full_js)

print(f'Generated: {out_path} ({len(full_js)} bytes)')
